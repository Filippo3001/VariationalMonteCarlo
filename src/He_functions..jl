...
function internucleusd(r)
    assert(length(r) == 12)
    # Extract from r the position coordinates
    r1 = r[1:3]
    r2 = r[4:6]
    r3 = r[7:9]
    r4 = r[9:12]
    # Evaluate the internucleons vectors
    r_12 = r1 - r2
    r_13 = r1 - r3
    r_14 = r1 - r4
    r_23 = r2 - r3
    r_24 = r2 - r4
    r_34 = r3 - r4
    # Return the internucleons distances
    return sqrt(r_12 ⋅ r_12), sqrt(r_13 ⋅ r_13), sqrt(r_14 ⋅ r_14), sqrt(r_23 ⋅ r_23), sqrt(r_24 ⋅ r_24), sqrt(r_34 ⋅ r_34) 
end
...
function Helocalen(par, x)
    assert(length(par) == 3)
    assert(length(r) == 12)

    Hehamiltonian(par, x) / Hetrialfunction(par, x)
end
