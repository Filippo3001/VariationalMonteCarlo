...
#Define a function which evaluate the integral using points generated by a different importancesamplings.
function corr_evaluate(points, oldimpsamplings, newimpsamplings, h)
    # Need to consider two evaluate (numerator and denominator)
    numerator(x) = (newimpsamplings(x) / oldimpsamplings(x)) * h(x)
    denominator(x) = newimpsamplings(x) / oldimpsamplings(x)

    mean_num, sigma_num = evaluate(points, numerator)
    mean_den, sigma_den = evaluate(points, denominator)

    new_mean = mean_num / mean_den
    #Use uncorrelated error propagation
    new_sigma = sqrt(sigma_num^2 + ((mean_num * sigma_den)^2 / mean_den^2)) / abs(mean_den)
    return new_mean, new_sigma
end

# Functions to eliminates
